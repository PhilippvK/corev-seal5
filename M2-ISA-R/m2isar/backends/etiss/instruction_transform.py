...
def assignment(self: behav.Assignment, context: TransformerContext):
	...
	if isinstance(self.target, behav.SliceOperation):
		...
		expr: CodeString = CodeString(f"((({target.code}) & {inv_shifted_mask}) | (({expr_} << {right}) & {shifted_mask}))", static and expr_.static, max(target.size, expr_.size), False, set())
	else:
