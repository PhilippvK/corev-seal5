...
def indexed_reference(self: behav.IndexedReference, context):
	# print("indexed_reference", self.index, self.reference, self.right)

	if isinstance(self.index, behav.NamedReference) and isinstance(self.reference, arch.Memory):
		# print("attrs", self.reference.attributes)
		if arch.MemoryAttribute.IS_MAIN_REG in self.reference.attributes:
			# print("index", self.index.reference.name, self.index.reference.data_type, self.index.reference.size)
			if ":" not in self.index.reference.name:
				index = self.index
				full_name = f"GPR:${self.index.reference.name}"
				ref = arch.Scalar(full_name, None, False, self.reference.size, arch.DataType.U)
				return behav.NamedReference(ref)

				# index.inferred_type = arch.IntegerType(self.reference.size, False, None)
				return index
			else:
				return self.index
