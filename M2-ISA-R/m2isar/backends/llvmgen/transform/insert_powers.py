...
def ternary(self: behav.Ternary, context):
	print("ternary", self, self.cond, self.then_expr, self.else_expr)
	if isinstance(self.cond, behav.BinaryOperation):
		op = self.cond.op.value
		detected = None
		if op in ["<", "<=", ">=", ">"]:
			print("cond", self.cond.left, self.cond.right)
			# TODO: ionfer sign of lhs and rhs?
			if self.cond.left == self.then_expr:  # TODO: needs structural equal?
				if self.cond.right == self.else_expr:
					if op in ["<", "<="]:
						detected = ("min", self.cond.left, self.cond.right)
					elif op in [">", ">="]:
						detected = ("max", self.cond.left, self.cond.right)
			elif self.cond.left == self.else_expr:
				if self.cond.right == self.then_expr:
					if op in ["<", "<="]:
						detected = ("min", self.cond.right, self.cond.left)
					elif op in [">", ">="]:
						detected = ("max", self.cond.right, self.cond.left)
			else:
				pass
