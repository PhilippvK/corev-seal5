...
def named_reference(self: behav.NamedReference, context):
	if isinstance(self.reference, arch.BitFieldDescr):
		if ":" not in self.reference.name:
			# print("BITFIELD", self.reference.name, self.reference.data_type, self.reference.size)
			sign_letter = "?"  # TODO: use DICT lookup
			if self.reference.data_type == arch.DataType.U:
				sign_letter = "u"
			elif self.reference.data_type == arch.DataType.S:
				sign_letter = "s"
			extra = ""
			# if self.reference.range.lower == 1:
			# 	extra += "_lsb0"
			self.reference.name = f"{sign_letter}imm{self.reference.size}{extra}:${self.reference.name}"
