...
def slice_operation(self: behav.SliceOperation, context):
	...
	if isinstance(self.left, behav.IntLiteral) and isinstance(self.right, behav.IntLiteral):
		...
		assert left >= right  # TODO
		# width = self.left - self.right + 1
		# WIDTH_TO_LETTER = {
		# 	8: "B",
		#	16: "H",
		#	32: "W",
		#	64: "D",
		#}
		# width_letter = WIDTH_TO_LETTER.get(width, None)
		...
		slice = LOOKUP_SLICES.get((left, right), None)
		letter, index = slice
		print("slice", slice)
		if slice is not None:
			self.annotations.append(f"{letter}[{index}]")
