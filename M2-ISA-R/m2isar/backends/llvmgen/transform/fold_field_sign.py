...
def type_conv(self: behav.TypeConv, context):
	# print("type_conv", self.data_type, self.size, self.expr)
	if isinstance(self.expr, behav.NamedReference):
		if isinstance(self.expr.reference, arch.BitFieldDescr):
			if self.size is None and self.data_type != self.expr.reference.data_type:
				expr = self.expr
				expr.reference.data_type = self.data_type
				return expr
			# print("ref", self.expr.reference.data_type, self.expr.reference.size, self.expr.reference.name)
