...
def binary_operation(self: behav.BinaryOperation, context):
	...
	if self.op.value == ">>":
		left_type = self.left.inferred_type
		if left_type is None:
			logger.warning("Detection of arithmetic shifts failed due to missing type information")
			return self
		assert isinstance(left_type, arch.IntegerType)
		if left_type.signed:
			self.op.value = ">>>"
