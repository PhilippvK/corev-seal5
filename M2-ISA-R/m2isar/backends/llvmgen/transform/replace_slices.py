...
def slice_operation(self: behav.SliceOperation, context):
	...
	if not isinstance(self.right, behav.IntLiteral):
		return self
	width = self.left.value - self.right.value + 1
	mask_value = (2 ** width) - 1


	if self.right.value == 0:

		self = behav.BinaryOperation(self.expr, behav.Operator("&"), behav.IntLiteral(mask_value))
	else:
		self = behav.BinaryOperation(behav.BinaryOperation(self.expr, behav.Operator(">>"), self.right), behav.Operator("&"), behav.IntLiteral(mask_value))
	# input("wait")
